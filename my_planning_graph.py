...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        '''
        # TODO add action A level to the planning graph as described in AIMA
        ...
        self.a_levels.append(set())  # Set of empty Action levels to start
        # For all Actions 
        for action in self.all_actions:
            # define an Action node
            node_a = PgNode_a(action)

            # For all State nodes in a given S-level that go through an Action node 
            # state node[level] -> action node[level]
            for node_s in self.s_levels[level]:
                # Add Action_nodes to that level, if node_a has pre-nodes that are a subset of the previous S level
                if set(node_a.prenodes).issubset(self.s_levels[level]):
                    self.a_levels[level].add(node_a)

                # Connect them up to their S level 
    ...
    def add_literal_level(self, level):
        ...
        '''
        # TODO add literal S level to the planning graph as described in AIMA
        ...
        self.s_levels.append(set()) # Empty S Levels to start

        # Want to associate the nodes in an A level right above a given S level 
        # action_node[level-1] -> state node[level]

        # For all action nodes above
        for node_a in self.a_levels[level-1]:
            for node_s in node_a.effnodes:
                # Add State node to current level
                self.s_levels[level].add(node_s)
                # Connect State nodes to the Action Node
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #  Check for a common element between node_a1 add and node_a2 remove & vice versa
        return (len(list(set(node_a1.action.effect_add) & set(node_a2.action.effect_rem))) >0 or len(list(set(node_a1.action.effect_rem) & set(node_a2.action.effect_add))) >0 )
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Look for common elemenets again, but this time between node_a1 add and node_2 precondition negation, and then vice versa
        return  ( 
        # adds
        len(list(set(node_a1.action.effect_add) & set(node_a2.action.precond_neg))) >0
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # TODO Test for Needs between nodes

        # For every parent in node_a1
        for node1_parent in node_a1.parents:
            # And every parent for node_a2
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        # Check if the S level symbols are the same, and check for negation
    ...
    def h_levelsum(self) -> int:
        ...
        level_count = len((self.s_levels)) # Get number of levels

        # For every Goal
        for each_goal in self.problem.goal:
            # Set up a Goal tracker and set it to false
            goal_tracker = False 
            # Go through every level
            for level in range(level_count):
                # if suddenly true, break; we are done
                if goal_tracker== True:
                    break
                else:
                     # for each State node check if its literal has the goal
                    for node_s in self.s_levels[level]:
